...
void loop() {
    ...
    // Slider value goes from 0 to 255.
    ...
    // If a text from the phone was sent print it to the serial monitor and send it back to the terminal on the phone
    if (str != "") {
        Serial.println(str);
        phone.sendMessage(str); // phone.sendMessage(str) sends the text to the phone.
        unsigned int in = 2;
        phone.sendDisplayData(in, str);
    }

    // Send string from serial command line to the phone. This will show up in the terminal.
    if (Serial.available()) {
        Serial.write("send: ");
